...
export class DailyExecutionComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.sumOpen[0] = sampleData.dailyExecutionJson[0].openCases;
    this.sumClosed[0] = sampleData.dailyExecutionJson[0].closedCases;

    for (let i = 0; i < 30; i++) {
      this.dates[i] = sampleData.dailyExecutionJson[i].date;
      this.openCases[i] = sampleData.dailyExecutionJson[i].openCases;
      this.closeCases[i] = sampleData.dailyExecutionJson[i].closedCases;
    }

    for (let i = 1; i < 30; i++) {
      this.sumOpen[i] = this.sumOpen[i - 1] + sampleData.dailyExecutionJson[i].openCases;
      this.sumClosed[i] = this.sumClosed[i - 1] + sampleData.dailyExecutionJson[i].closedCases;
    }
